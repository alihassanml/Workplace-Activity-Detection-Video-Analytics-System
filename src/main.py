...
def find_best_match(detection_bbox, tracked_persons, matched_ids):
    ...
    for pid, pdata in tracked_persons.items():
        ...
        # Weight IoU more heavily, but allow distance to help when IoU is low
        combined_score = (iou * 0.6) + (dist_score * 0.4)
        
        # Bonus for temporal consistency (recently seen tracks get priority)
        if frames_lost == 0:
            combined_score *= 1.2
...
while True:
    ...
    if annotated_frame is not None:
        ...
        for person_id, pdata in display_tracked_persons.items():
            ...
            # Dim color if person is temporarily lost
            frames_lost = pdata.get('frames_lost', 0)
            if frames_lost > 0:
